...
#define FLANK_THRESHOLD_min 0.2
//#define FLANK_THRESHOLD_max 100
...
int main(int argc, char **argv){
  ...
  string posearray_op = "/edge_leg_detector";
  ros::param::get("~laser_scan", laser_scan);
  ros::param::get("~posearray_op", posearray_op);


  ros::NodeHandle n;
  ros::Publisher  node_pub = n.advertise <geometry_msgs::PoseArray>(posearray_op, 2); // Humans in the environment
  ...
  while( ros::ok() ){
    if( sensor_on == true ){
      ...
      void ValidateDistance();
      ROS_INFO_STREAM("size::"<<rec_x.size());
...
void LaserCallback (const sensor_msgs::LaserScan::ConstPtr& msg){
  ...
  for( unsigned i = 1; i < laser_flank.size(); i++ ){
    float flank_measure = fabs( laser_r[ i ] - laser_r[ i - 1 ]);
    //if(!gg)
      //ROS_INFO_STREAM("Flank:"<<flank_measure);
    if(flank_measure>FLANK_THRESHOLD_min)//&&(flank_measure<FLANK_THRESHOLD_max))
       {
...
void ValidatePattern( list <int> *Pattern_list, int TYPE,  vector <int> flank_id0,  vector <int> flank_id1, vector <double> laser_x, vector <double> laser_y){
  ...
  for( it = (*Pattern_list).begin(); it != (*Pattern_list).end(); it++ ){
	...
	cond_c = true;
    //ROS_INFO_STREAM("Antro:::"<<float(ANTRO_a_1)<<"*"<<float(ANTRO_a_2)<<"*"<<float(ANTRO_b)<<"*"<<ANTRO_c<<"*"<<TYPE);
        break;
    ...
    if( !SavePattern ){
      ...
      it--;
        ...
        //ROS_INFO_STREAM("Antro:::"<<float(ANTRO_a_1)<<"*"<<float(ANTRO_a_2)<<"*"<<float(ANTRO_b)<<"*"<<TYPE);
    }
